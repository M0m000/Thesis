...
class HookFilter:
    def __init__(self, alpha=0.2, confirmation_frames=3, disappearance_frames=1):
    ...
    def update(self, hooks_dict):
        ...
        for hook_key, hook_data in hooks_dict.items():
            if hook_key not in self.hook_history:
                # Neuer Haken: Initialisiere Historie (inkl. UV und Bounding Box)
            ...
            # Bounding Box-Koordinaten filtern (angenommen im Format (x, y, w, h))
        ...
        # Entferne Haken, die im aktuellen hooks_dict nicht vorhanden sind (schnelles Entfernen)
