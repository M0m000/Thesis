...
class VCImage2Receiver(Node):
    def __init__(self):
        ...
        self.image_publisher = self.create_publisher(Image, 'vcnanoz/image_raw_2', 10)
        self.get_logger().info(f'IPv4: {self.ipv4} - Port: {self.port}')
        self.image_width_publisher = self.create_publisher(Int32, 'vcnanoz/image_raw_2/width', 10)
        self.image_height_publisher = self.create_publisher(Int32, 'vcnanoz/image_raw_2/height', 10)
        
        # Versuche kontinuierlich zu verbinden
        self.connect()


    def width_received_callback(self, msg):
        self.dx = msg.data
    
    def height_received_callback(self, msg):
        self.dy = msg.data
