...
class YOLOv8InferenceNode(Node):
            ...
            for j, box_hook2 in enumerate(boxes_hooks):
                if i != j:  # Keine Distanz zu sich selbst berechnen
                    hook_distances[i, j] = np.linalg.norm(
                        self.calc_box_midpoint(box_hook1) - self.calc_box_midpoint(box_hook2)
                    )
    ...
    def create_bar_instance(self, boxes_bar, masks_bar, confs_bar):
        ...
        box_bar = boxes_bar[np.argmax(confs_bar)]
        mask_bar = masks_bar[np.argmax(confs_bar)]
        conf_bar = confs_bar[np.argmax(confs_bar)]
    ...
    def plot_hooks_and_bars(self, img_orig, hooks_dict, bar_dict):
        ...
        # Zeichne jeden Haken anderer Farbe
