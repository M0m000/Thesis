...
class Yolov8Seg:
    def __init__(self, onnx_model, yolo_yaml_filepath):
        ...
        self.model_batchsize, self.model_depth, self.model_height, self.model_width = self.session.get_inputs()[0].shape
        self.classes = yaml_load(check_yaml(yolo_yaml_filepath))['names']
    ...
    def __call__(self, im0, conf_threshold=0.4, iou_threshold=0.45, nm=32):
        ...
        ### Preprocess
        pre_starttime = time.perf_counter()
        im, ratio, (pad_w, pad_h) = self.preprocess(im0)
        pre_endtime = time.perf_counter()
        time_values.append((pre_endtime - pre_starttime))

        ### Inference
        inf_starttime = time.perf_counter()
        preds = self.session.run(None, {self.session.get_inputs()[0].name: im})
        inf_endtime = time.perf_counter()
        time_values.append((inf_endtime - inf_starttime))

        ### Postprocess
        post_starttime = time.perf_counter()
        ...
        post_endtime = time.perf_counter()
        time_values.append((post_endtime - post_starttime))
        
        return boxes, segments, masks, time_values
    ...
    def draw_and_visualize(self, im, bboxes, segments, vis=False, save=True):
        ...
        # Save image
        if save:
            cv2.imwrite("inference_output.png", im)