...
class ScanBarCombinedTriangulation(Node):
    def __init__(self):
        ...
        startpoint_trans_in_workframe = [150.0, -430.0, 20.0]
        ...
        self.T_cam_in_workframe_vertical = None
        self.T_cam_in_worldframe_vertical = None
    ...
    def process_main(self):
        ...
        if self.process_step == "move_vertical":
            """
            Bewegung in vertikaler Richtung für vertikale Triangulation
            """
            if self.yolo_hooks_dict['hook_2']['uv_lowpoint'][1] < (self.img_height * 0.7):
        ...
        if self.process_step == "extract_vertical_hook":
            ...
            if self.hook_extraction_done == True:
                # CAM Pose in WORK speichern
                self.T_cam_in_workframe_vertical = self.frame_handler.get_cam_transform_in_workframe()
                self.T_cam_in_worldframe_vertical = self.frame_handler.get_cam_transform_in_world()
                self.get_logger().info("Done! -> next process step <Move until new hook appears>")
        ...
        if self.process_step == "move_back_to_ref_hook":
            ...
            if self._help_movement_service_called == False:
                self._help_movement_done = self.move_linear_client.call_move_linear_service(
                    pos = self.tfc_in_worldframe_ref_trans,
                    rot = self.tfc_in_worldframe_ref_rot,
                    ref = 0,
                    ttype = 0,
                    tvalue = 50.0,
                    bpoint = 0,
        ...
        if self.process_step == "combined_triangulation":
            ...
            if baseline_along_x == 0 or baseline_along_y == 0:
                ...
                xyz_hook_in_camframe_horizontal, time1_horizontal = self.triangulation_processor.triangulate_single_point(
                ...
                xyz_tip_in_camframe_horizontal, time2_horizontal = self.triangulation_processor.triangulate_single_point(
                    point_1_uv = self.hook_ref['uv_tip'], 
                    point_2_uv = self.hook_horizontal['uv_tip'], 
                    T_cam_1_in_workframe = self.T_cam_in_workframe_ref, 
                    T_cam_2_in_workframe = self.T_cam_in_workframe_horizontal)
                
                xyz_lowpoint_in_camframe_horizontal, time3_horizontal = self.triangulation_processor.triangulate_single_point(
                    point_1_uv = self.hook_ref['uv_lowpoint'], 
                    point_2_uv = self.hook_horizontal['uv_lowpoint'], 
                    T_cam_1_in_workframe = self.T_cam_in_workframe_ref, 
                    T_cam_2_in_workframe = self.T_cam_in_workframe_horizontal)
                
                xyz_hook_in_camframe_vertical, time1_vertical = self.triangulation_processor.triangulate_single_point(
                    point_1_uv = self.hook_ref['uv_hook'], 
                    point_2_uv = self.hook_vertical['uv_hook'], 
                    T_cam_1_in_workframe = self.T_cam_in_workframe_ref, 
                    T_cam_2_in_workframe = self.T_cam_in_workframe_vertical)
                
                xyz_tip_in_camframe_vertical, time2_vertical = self.triangulation_processor.triangulate_single_point(
                    point_1_uv = self.hook_ref['uv_tip'], 
                    point_2_uv = self.hook_vertical['uv_tip'], 
                    T_cam_1_in_workframe = self.T_cam_in_workframe_ref, 
                    T_cam_2_in_workframe = self.T_cam_in_workframe_vertical)
                
                xyz_lowpoint_in_camframe_vertical, time3_vertical = self.triangulation_processor.triangulate_single_point(
                    point_1_uv = self.hook_ref['uv_lowpoint'], 
                    point_2_uv = self.hook_vertical['uv_lowpoint'], 
                    T_cam_1_in_workframe = self.T_cam_in_workframe_ref, 
                    T_cam_2_in_workframe = self.T_cam_in_workframe_vertical)
                
                time_token = time1_horizontal + time2_horizontal + time3_horizontal + time1_vertical + time2_vertical + time3_vertical      # Berechnungszeiten aller drei Punkten addieren
                ...
                if uv_path_points_ref is not None and uv_path_points_horizontal is not None and uv_path_points_vertical is not None:
                    self.path_point_triangulation_successful = True
                    xyz_path_points_in_camframe_horizontal, time_token_ppoint_triangulation_horizontal = self.triangulation_processor.triangulate_path_points(                    
                        uv_path_points_1 = uv_path_points_ref, 
                        uv_path_points_2 = uv_path_points_horizontal,
                        T_cam_1_in_workframe = self.T_cam_in_workframe_ref, 
                        T_cam_2_in_workframe = self.T_cam_in_workframe_horizontal)
                    
                    xyz_path_points_in_camframe_vertical, time_token_ppoint_triangulation_vertical = self.triangulation_processor.triangulate_path_points(                    
                        uv_path_points_1 = uv_path_points_ref, 
                        uv_path_points_2 = uv_path_points_vertical,
                        T_cam_1_in_workframe = self.T_cam_in_workframe_ref, 
                        T_cam_2_in_workframe = self.T_cam_in_workframe_vertical)
                    ...
                    time_token_ppoint_triangulation = time_token_ppoint_triangulation_horizontal + time_token_ppoint_triangulation_vertical
                else:
                    self.xyz_path_points_in_camframe = None
                    self.path_point_triangulation_successful = False
        ...
        if self.process_step == "save_hook":
            ...
            xyz_hook_in_camframe_horizontal_hom = np.array([xyz_hook_in_camframe_horizontal[0], xyz_hook_in_camframe_horizontal[1], xyz_hook_in_camframe_horizontal[2]]).reshape(3, 1)
            xyz_hook_in_camframe_horizontal_hom = np.vstack((xyz_hook_in_camframe_horizontal_hom, [[1]]))  # -> (4, 1)
            xyz_hook_in_workframe_horizontal = self.T_cam_in_workframe_horizontal @ xyz_hook_in_camframe_horizontal_hom               # Umrechnung in WORK
            xyz_hook_in_workframe_horizontal = xyz_hook_in_workframe_horizontal[:3]
            xyz_hook_in_worldframe_horizontal = self.T_cam_in_worldframe_horizontal @ xyz_hook_in_camframe_horizontal_hom             # Umrechnung in WORLD
            xyz_hook_in_worldframe_horizontal = xyz_hook_in_worldframe_horizontal[:3]

            xyz_tip_in_camframe_horizontal_hom = np.array([xyz_tip_in_camframe_horizontal[0], xyz_tip_in_camframe_horizontal[1], xyz_tip_in_camframe_horizontal[2]]).reshape(3, 1)
            xyz_tip_in_camframe_horizontal_hom = np.vstack((xyz_tip_in_camframe_horizontal_hom, [[1]]))  # -> (4, 1)
            xyz_tip_in_workframe_horizontal = self.T_cam_in_workframe_horizontal @ xyz_tip_in_camframe_horizontal_hom                 # Umrechnung in WORK
            xyz_tip_in_workframe_horizontal = xyz_tip_in_workframe_horizontal[:3]
            xyz_tip_in_worldframe_horizontal = self.T_cam_in_worldframe_horizontal @ xyz_tip_in_camframe_horizontal_hom               # Umrechung in WORLD
            xyz_tip_in_worldframe_horizontal = xyz_tip_in_worldframe_horizontal[:3]

            xyz_lowpoint_in_camframe_horizontal_hom = np.array([xyz_lowpoint_in_camframe_horizontal[0], xyz_lowpoint_in_camframe_horizontal[1], xyz_lowpoint_in_camframe_horizontal[2]]).reshape(3, 1)
            xyz_lowpoint_in_camframe_horizontal_hom = np.vstack((xyz_lowpoint_in_camframe_horizontal_hom, [[1]]))  # -> (4, 1)
            xyz_lowpoint_in_workframe_horizontal = self.T_cam_in_workframe_horizontal @ xyz_lowpoint_in_camframe_horizontal_hom       # Umrechnung in WORK
            xyz_lowpoint_in_workframe_horizontal = xyz_lowpoint_in_workframe_horizontal[:3]
            xyz_lowpoint_in_worldframe_horizontal = self.T_cam_in_worldframe_horizontal @ xyz_lowpoint_in_camframe_horizontal_hom     # Umrechung in WORLD
            xyz_lowpoint_in_worldframe_horizontal = xyz_lowpoint_in_worldframe_horizontal[:3]

            xyz_hook_in_camframe_vertical_hom = np.array([xyz_hook_in_camframe_vertical[0], xyz_hook_in_camframe_vertical[1], xyz_hook_in_camframe_vertical[2]]).reshape(3, 1)
            xyz_hook_in_camframe_vertical_hom = np.vstack((xyz_hook_in_camframe_vertical_hom, [[1]]))  # -> (4, 1)
            xyz_hook_in_workframe_vertical = self.T_cam_in_workframe_vertical @ xyz_hook_in_camframe_vertical_hom               # Umrechnung in WORK
            xyz_hook_in_workframe_vertical = xyz_hook_in_workframe_vertical[:3]
            xyz_hook_in_worldframe_vertical = self.T_cam_in_worldframe_vertical @ xyz_hook_in_camframe_vertical_hom             # Umrechnung in WORLD
            xyz_hook_in_worldframe_vertical = xyz_hook_in_worldframe_vertical[:3]

            xyz_tip_in_camframe_vertical_hom = np.array([xyz_tip_in_camframe_vertical[0], xyz_tip_in_camframe_vertical[1], xyz_tip_in_camframe_vertical[2]]).reshape(3, 1)
            xyz_tip_in_camframe_vertical_hom = np.vstack((xyz_tip_in_camframe_vertical_hom, [[1]]))  # -> (4, 1)
            xyz_tip_in_workframe_vertical = self.T_cam_in_workframe_vertical @ xyz_tip_in_camframe_vertical_hom                 # Umrechnung in WORK
            xyz_tip_in_workframe_vertical = xyz_tip_in_workframe_vertical[:3]
            xyz_tip_in_worldframe_vertical = self.T_cam_in_worldframe_vertical @ xyz_tip_in_camframe_vertical_hom               # Umrechung in WORLD
            xyz_tip_in_worldframe_vertical = xyz_tip_in_worldframe_vertical[:3]

            xyz_lowpoint_in_camframe_vertical_hom = np.array([xyz_lowpoint_in_camframe_vertical[0], xyz_lowpoint_in_camframe_vertical[1], xyz_lowpoint_in_camframe_vertical[2]]).reshape(3, 1)
            xyz_lowpoint_in_camframe_vertical_hom = np.vstack((xyz_lowpoint_in_camframe_vertical_hom, [[1]]))  # -> (4, 1)
            xyz_lowpoint_in_workframe_vertical = self.T_cam_in_workframe_vertical @ xyz_lowpoint_in_camframe_vertical_hom       # Umrechnung in WORK
            xyz_lowpoint_in_workframe_vertical = xyz_lowpoint_in_workframe_vertical[:3]
            xyz_lowpoint_in_worldframe_vertical = self.T_cam_in_worldframe_vertical @ xyz_lowpoint_in_camframe_vertical_hom     # Umrechung in WORLD
            xyz_lowpoint_in_worldframe_vertical = xyz_lowpoint_in_worldframe_vertical[:3]

            ##### Mittelwert der beiden Triangulationsergebnisse im WORK-Frame berechnen
            xyz_hook_in_workframe = (xyz_hook_in_workframe_horizontal + xyz_hook_in_workframe_vertical) / 2
            xyz_hook_in_worldframe = (xyz_hook_in_worldframe_horizontal + xyz_hook_in_worldframe_vertical) / 2
            xyz_tip_in_workframe = (xyz_tip_in_workframe_horizontal + xyz_tip_in_workframe_vertical) / 2
            xyz_tip_in_worldframe = (xyz_tip_in_worldframe_horizontal + xyz_tip_in_worldframe_vertical) / 2
            xyz_lowpoint_in_workframe = (xyz_lowpoint_in_workframe_horizontal + xyz_lowpoint_in_workframe_vertical) / 2
            xyz_lowpoint_in_worldframe = (xyz_lowpoint_in_worldframe_horizontal + xyz_lowpoint_in_worldframe_vertical) / 2


            ##### Umrechnung der XYZ Path Points vom CAM-Frame ins WORK-Frame
            xyz_path_points_in_workframe_horizontal = []
            xyz_path_points_in_workframe_vertical = []
            xyz_path_points_in_workframe = []

            if xyz_path_points_in_camframe_horizontal is not None and xyz_path_points_in_camframe_vertical is not None:
                for ppoint in xyz_path_points_in_camframe_horizontal:
                    ppoint_xyz_hom = np.append(ppoint, 1)
                    xyz_ppoint_in_workframe = self.T_cam_in_workframe_horizontal @ ppoint_xyz_hom
                    xyz_path_points_in_workframe_horizontal.append(xyz_ppoint_in_workframe[:3])
                for ppoint in xyz_path_points_in_camframe_vertical:
                    ppoint_xyz_hom = np.append(ppoint, 1)
                    xyz_ppoint_in_workframe = self.T_cam_in_workframe_vertical @ ppoint_xyz_hom
                    xyz_path_points_in_workframe_vertical.append(xyz_ppoint_in_workframe[:3])
                for idx in range(len(xyz_path_points_in_workframe_horizontal)):
                    xyz_path_points_in_workframe.append((xyz_path_points_in_workframe_horizontal[idx] + xyz_path_points_in_workframe_vertical[idx]) / 2)


            ##### Speichern der Koordinaten in restlichen Frames CAM, WORLD
            self.global_hooks_dict[str(self.act_hook_num)]['xyz_hook_in_camframe'] = xyz_hook_in_camframe_horizontal
            self.global_hooks_dict[str(self.act_hook_num)]['xyz_tip_in_camframe'] = xyz_tip_in_camframe_horizontal
            self.global_hooks_dict[str(self.act_hook_num)]['xyz_lowpoint_in_camframe'] = xyz_lowpoint_in_camframe_horizontal
